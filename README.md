# Hi, I'm Natol Tesfaye 👋

### Software Engineering Student | Full-Stack Developer | Java & Web Development

I'm a Software Engineering undergraduate interested in building practical, reliable, and user-friendly software.

I enjoy working across both frontend and backend development and continuously improving my programming and software engineering skills.

---

## 🚀 About Me

- 🎓 Software Engineering undergraduate
- 💻 Interested in Full-Stack Development
- ☕ Working with Java and Object-Oriented Programming
- 🌐 Learning and building web applications
- 🧩 Interested in solving real-world problems through software
- 📚 Continuously learning new technologies and development practices

---

## 🛠️ Technologies & Skills

### Programming
- Java
- JavaScript

### Frontend
- HTML
- CSS
- JavaScript
- React

### Backend
- Node.js

### Software Engineering
- Object-Oriented Programming
- Web Development
- Backend Development
- Frontend Development

---

## 📌 Featured Projects

### ✈️ Flight Ticket Booking System
A Java-based flight ticket booking application focused on booking management, seat availability, and administrative functionality.

### 🔳 QR Business Card System
A web-based project for creating and managing digital business cards using QR technology.

---

## 🎯 Current Goals

- Build more advanced full-stack projects
- Strengthen my software engineering skills
- Improve my problem-solving abilities
- Create practical projects for my portfolio
- Learn and apply modern development practices

---

## 📫 Connect With Me

- GitHub: [@natoltesfaye](https://github.com/natoltesfaye)

---

⭐ Thanks for visiting my profile!
